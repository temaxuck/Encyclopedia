...
from encyclopedia.forms import (
    UploadPyramidForm,
    ConfirmPyramidDeletionForm,
    RelationSubform,
    ExplicitFormulaSubform,
    GeneratingFunctionSubform,
)
...
def pyramid(snid: int):
    ...
    if deleteform.validate_on_submit():
        if current_user.moderator:
            ...
            flash(
                f"The pyramid #{pyramid.sequence_number} has been successfully deleted!",
                "success",
            )
    ...
    return render_template(
        "pyramid.html", pyramid=pyramid, PyramidModel=Pyramid, deleteform=deleteform
    )
...
def pyramid_list():
    ...
    return render_template(
        "pyramid_list.html", pyramids=pyramids, page=page, pages_in_total=pages_in_total
    )
...
def upload_pyramid():
    ...
    if form.validate_on_submit():
        try:
            alreadyExist = (
                Pyramid.query.filter_by(
                    sequence_number=form.sequenceNumber.data
                ).count()
                > 0
            )
            if alreadyExist:
            ...
            for i, gf in enumerate(form.generatingFunction):
                ...
                if (
                    gf.f_name.data == "U"
                    or gf.f_name.data == "u"
                    or i == len(form.generatingFunction) - 1
                ):
                    isMain = True
                pyramid.add_generating_function(
                    gf.f_name.data, gf.f_vars.data, gf.f_expr.data, isMain
                )

            for ef in form.explicitFormula:
                pyramid.add_explicit_formula(
                    form.ef_vars.data, ef.f_expr.data, ef.f_condition.data
                )

            pyramid.init_special_value()
            alreadyExist = (
                Pyramid.query.filter_by(
                    __special_hashed_value__=pyramid.__special_hashed_value__
                ).count()
                > 0
            )
            ...
            if alreadyExist:
                flash(
                    f"Pyramid with such formulas probably already exists. Make sure generating function and explicit formula are unique",
                    "danger",
                )
                return redirect(url_for("pyramid.upload_pyramid"))
            ...
            for i, relation in enumerate(form.relations.data):
                try:
                    pyramid.add_relation(
                        Pyramid.query.filter_by(
                            sequence_number=relation.get("relatedto_pyramid")
                        )
                        .first()
                        .id,
                        relation.get("tag"),
                    ...
                    flash(
                        f"Could not add a relation between pyramids #{relation.get('relatedto_pyramid')} and #{pyramid.sequence_number}",
                        "danger",
                    )
                    return redirect(
                        url_for("pyramid.edit_pyramid", snid=pyramid.sequence_number)
                    )
...
def edit_pyramid(snid: int):
    ...
    if form.validate_on_submit():
        ...
        if pyramid.generating_function:
            form.generatingFunction[0].f_name.data = pyramid.generating_function[
                0
            ].function_name
            form.generatingFunction[0].f_vars.data = pyramid.generating_function[
                0
            ].get_variables_as_str()
            form.generatingFunction[0].f_expr.data = pyramid.generating_function[
                0
            ].expression
        ...
        if pyramid.explicit_formula:
            ...
            form.explicitFormula[0].f_condition.data = pyramid.explicit_formula[
                0
            ].limitation
